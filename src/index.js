...
async function getpost(id, page = null) {
  ...
  if (response.status === 403) return { topic_id: topicid, author: null, bbcodesource: null, is404: false, isdustbinned: true };
  ...
  return { topic_id: topicid, author, bbcodesource: await sourcerequest, is404: false, isdustbinned: false };
}
...
async function cacheforever() {
  while (true) {
    try { await getpostcaching(await getnextparent()); } 
    catch(e){ console.error("Worker loop error:", e); await sleep(1000); }
    await sleep(500);
  }
}
...
async function cache() {
  for (let i=0;i<10;i++) {
    ...
    w.on("message", async () => { try { w.postMessage(await getnext(true)); } catch(e){ console.error("Worker message error:", e); } });
    w.on("error", e=>console.error("Worker error:", e));
    w.on("exit", c=>{ if(c!==0) console.error(`Worker ${i} exited ${c}`); });
    await sleep(1000);
  }
}
